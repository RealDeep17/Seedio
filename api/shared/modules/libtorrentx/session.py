...
class LibTorrentSession(MagnetUtils):
    def __init__(self, redis=None):
        ...
        settings.update(
            {
                ...
                "upload_rate_limit": 1,  # 1 byte/s = effectively no upload (no seeding back)
            }
        )
        ...
        # ── Protocol Encryption (MSE/PE) — Tiered ────────────────────────────
        # Prefer RC4-encrypted peers first; if a peer doesn't support
        # encryption, still connect (unencrypted fallback). This maximises
        # peer availability for rare/old torrents while keeping OCI DPI
        # from reading traffic on all peers that DO support it.
        if hasattr(lt, "pe_settings"):
            pe = lt.pe_settings()
            pe.out_enc_policy = lt.pe_settings.enc_policy.enabled  # prefer encrypted, allow plaintext fallback
            pe.in_enc_policy  = lt.pe_settings.enc_policy.enabled  # prefer encrypted, allow plaintext fallback
            pe.allowed_enc_level = lt.pe_settings.enc_level.rc4    # RC4 full-stream when encrypting
            pe.prefer_rc4 = True
